...
if (isset($_GET['ajax']) && $_GET['ajax'] == 'get_booked_slots') {
    ...
    // Запрашиваем все занятые слоты для этого мастера на выбранную дату
...
<head>
    ...
    <style>
        ...
        /* Для отладки - показываем количество занятых слотов */
        .debug-info {
            font-size: 11px;
            color: #666;
            margin-top: 5px;
                    ...
                    <div class="card-body">
                        ...
                        <div class="mb-3">
                            ...
                            <div id="timeSlotsContainer" class="row g-2">
                                <!-- Слоты времени будут созданы через JS -->
                            </div>
...
// Функция: получаем занятые слоты для мастера и даты
async function getBookedSlots(masterId, date) {
    ...
    try {
        ...
        // Нормализуем время: обрезаем секунды ("09:00:00" -> "09:00")
...
// Функция: создаем ячейки времени на основе занятых слотов
async function renderTimeSlots() {
    const container = document.getElementById('timeSlotsContainer');
    const warning = document.getElementById('noMasterWarning');
    const loading = document.getElementById('loadingIndicator');
    
    // Если мастер не выбран - показываем предупреждение
    if (!currentMasterId) {
    ...
    // Запрашиваем занятые слоты для выбранного мастера и даты
    ...
    // Создаем ячейки для каждого возможного времени
    allTimeSlots.forEach(slot => {
        ...
        if (isBooked) {
            btnClass += 'booked';  // серая, зачеркнутая, disabled
        } else if (isSelected) {
            btnClass += 'selected'; // зеленая заливка
        } else {
            btnClass += 'available'; // зеленая рамка
        }
