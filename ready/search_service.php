...
/**
 * Разбиение ОДНОГО слова на слоги (по гласным)
 */
...
/**
 * Разбиение ПРЕДЛОЖЕНИЯ на слоги (сначала на слова, потом каждое слово на слоги)
 */
...
/**
 * Генерация всех уникальных комбинаций индексов
 */
...
/**
 * Подсчет количества услуг, содержащих ВСЕ указанные слоги (AND)
 */
...
/**
 * Получение списка услуг, содержащих ВСЕ указанные слоги (AND)
 */
...
if (!empty($search_query)) {
    // Используем новую функцию для разбиения предложения
    ...
    // НОВАЯ ЛОГИКА: выбираем комбинацию с НАИБОЛЬШИМ количеством слогов среди тех, у кого count > 0
    $bestServices = [];
    ...
    foreach ($results as $r) {
        if ($r['count'] > 0) {
            // Сначала по минимальному количеству найденных услуг
            // Потом по максимальному количеству слогов
